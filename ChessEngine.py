...
def ev_func(board):
    ...
    return math.tanh(A * material(board) + B * cent_cont(board) + C * kingSafety(board) + D * mobiltity(board))
...
def grad(board):
    # Gradient of J(x)
    return [material(board), cent_cont(board), kingSafety(board), mobiltity(board)]
